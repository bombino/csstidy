            ...
            if(csstidy::is_token($string,$i))
            {
                ...
                elseif($string{$i} == '@' && trim($this->selector) == '')
                {
                    ...
                    foreach($at_rules as $name => $type)
                    {
                        ...
                        {
                            ($type == 'at') ? $this->at = '@'.$name : $this->selector = '@'.$name."RANDFIX".rand();
                            $this->status = $type;
                            $i += strlen($name);
                    ...
                    if($this->sub_value != '') {
                    	...
                    	if (strstr($this->selector, "font-face")) {
                    	  foreach($this->sub_value_arr as &$sub_value) {
                    	    foreach (array('url','format') as $fix) {
                    		    if (substr($sub_value, 0, strlen($fix)) == $fix) {
                    			    $sub_value = str_replace(array($fix . '(', ')'), array($fix . "('", "')"), $sub_value);
                    	 	    }
                    	 	  }
                    	  }
                    	}
